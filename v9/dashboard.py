...
def compare_pages(df1, df2, top_n=10):
    ...
    # Top páginas de cada período
    pages1 = df1.nlargest(top_n, 'Vista de Páginas')[['Direccion de Página', 'Vista de Páginas']]
    pages2 = df2.nlargest(top_n, 'Vista de Páginas')[['Direccion de Página', 'Vista de Páginas']]
    
    comparison = {}
    all_pages = set(list(pages1['Direccion de Página']) + list(pages2['Direccion de Página']))
    
    for page in all_pages:
        p1_views = pages1[pages1['Direccion de Página'] == page]['Vista de Páginas'].sum()
        p2_views = pages2[pages2['Direccion de Página'] == page]['Vista de Páginas'].sum()
    ...
    # Ordenar por cambio más significativo
    sorted_comparison = dict(sorted(comparison.items(), key=lambda x: abs(x[1]['change_pct']), reverse=True))
...
def compare_search_terms(df1, df2, top_n=10):
    ...
    terms1 = df1.nlargest(top_n, 'Sesiones')[['Término de Búsqueda', 'Sesiones']]
    terms2 = df2.nlargest(top_n, 'Sesiones')[['Término de Búsqueda', 'Sesiones']]
    
    comparison = {}
    all_terms = set(list(terms1['Término de Búsqueda']) + list(terms2['Término de Búsqueda']))
    
    for term in all_terms:
        p1_sessions = terms1[terms1['Término de Búsqueda'] == term]['Sesiones'].sum()
        p2_sessions = terms2[terms2['Término de Búsqueda'] == term]['Sesiones'].sum()
...
def compare_geography(df1, df2, top_n=10):
    ...
    countries1 = df1.groupby('Pais')['Usuarios Activos'].sum().nlargest(top_n)
    countries2 = df2.groupby('Pais')['Usuarios Activos'].sum().nlargest(top_n)
...
def compare_traffic_sources(df1, df2, top_n=10):
    ...
    sources1 = df1.nlargest(top_n, 'Sesiones')[['Fuente/Medio', 'Sesiones', 'Usuarios Activos']]
    sources2 = df2.nlargest(top_n, 'Sesiones')[['Fuente/Medio', 'Sesiones', 'Usuarios Activos']]
    
    comparison = {}
    all_sources = set(list(sources1['Fuente/Medio']) + list(sources2['Fuente/Medio']))
    
    for source in all_sources:
        p1_sessions = sources1[sources1['Fuente/Medio'] == source]['Sesiones'].sum()
        p2_sessions = sources2[sources2['Fuente/Medio'] == source]['Sesiones'].sum()
