...
def insert_canales_marketing(connection, df, fecha):
    ...
    for _, row in df.iterrows():
        try:
            query = """
                INSERT IGNORE INTO canales_marketing 
                (fecha, channelGrouping, sourceMedium, campaignName, activeUsers, sessions, screenPageViews, bounceRate, averageSessionDuration)
            ...
            logging.error(f"Error insertando canal marketing: {e}")
...
def insert_palabras_clave(connection, df, fecha):
    ...
    for _, row in df.iterrows():
        try:
            query = """
                INSERT IGNORE INTO palabras_clave 
...
def insert_eventos_personalizados(connection, df, fecha):
    ...
    for _, row in df.iterrows():
        try:
            query = """
                INSERT IGNORE INTO eventos_personalizados 
