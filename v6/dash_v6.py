...
TABLE_CONFIG = {
    ...
    'fuentes_trafico': 'fuentes_trafico',  # ASEGURAR QUE ESTÉ PRESENTE
...
def create_connection():
    try:
        ...
        if connection.is_connected():
            ...
            return connection  # AGREGAR ESTA LÍNEA
    except mysql.connector.Error as e:  # CAMBIAR EXCEPCIÓN
...
# =============================================================================
# 🔍 FUNCIONES DE TRACKING (aun no funcionan kkk)
# =============================================================================

def get_client_info():
    """Obtener información del cliente usando JavaScript y Streamlit"""
    # Obtener IP desde headers de Streamlit
    try:
        # Streamlit puede proporcionar headers de la request
    ...
    # Obtener User Agent
    try:
        user_agent = headers.get('user-agent', 'unknown') if 'headers' in locals() else 'unknown'
    except:
        user_agent = 'unknown'
...
def parse_user_agent(user_agent_string):
    """Parsear el user agent para obtener información del dispositivo"""
    try:
...
def get_or_create_session_id():
    ...
    if 'session_id' not in st.session_state:
        # Crear un ID único basado en timestamp y datos del usuario
        timestamp = datetime.now().isoformat()
        client_info = get_client_info()
        unique_string = f"{timestamp}_{client_info['ip_address']}_{client_info['user_agent']}"
        st.session_state.session_id = hashlib.md5(unique_string.encode()).hexdigest()
...
def track_user_session(db):
    """Registrar o actualizar sesión de usuario"""
    try:
        ...
        check_session_sql = f"""
        SELECT id, total_visits, first_access 
        FROM {TABLE_CONFIG['user_sessions']} 
        WHERE session_id = %s
        """
        
        existing_session = db.query(check_session_sql, params=(session_id,))
        
        if existing_session.empty:
            # Nueva sesión
            insert_sql = f"""
            INSERT INTO {TABLE_CONFIG['user_sessions']} (
                session_id, ip_address, user_agent, device_type, 
                browser_name, browser_version, operating_system,
                pages_viewed, total_visits
            ) VALUES (
                %s, %s, %s, %s, %s, %s, %s, %s, %s
            )
            """
            ...
            update_sql = f"""
            UPDATE {TABLE_CONFIG['user_sessions']} 
            SET 
        ...
        st.error(f"Error tracking session: {e}")
        return False

def get_session_analytics(db, hours_back=24):
    """Obtener analytics de sesiones de usuarios"""
    try:
        # Sesiones activas en las últimas X horas
        ...
        FROM {TABLE_CONFIG['user_sessions']} 
        WHERE last_access >= DATE_SUB(NOW(), INTERVAL %s HOUR)
        """
        
        # Distribución por dispositivos
        device_distribution_sql = f"""
        SELECT 
            device_type,
        ...
        SELECT 
            ...
            COUNT(DISTINCT session_id) as unique_sessions,
            SUM(pages_viewed) as page_views
        FROM {TABLE_CONFIG['user_sessions']} 
        WHERE last_access >= DATE_SUB(NOW(), INTERVAL %s HOUR)
        ...
        active_stats = db.query(active_sessions_sql, params=(hours_back,))
        device_stats = db.query(device_distribution_sql, params=(hours_back,))
        browser_stats = db.query(browser_stats_sql, params=(hours_back,))
        hourly_stats = db.query(hourly_activity_sql, params=(hours_back,))
...
def show_current_session_info():
    """Mostrar información de la sesión actual (para debugging)"""
    if st.checkbox("🔍 Mostrar info de sesión actual (Debug)"):
        client_info = get_client_info()
        device_info = parse_user_agent(client_info['user_agent'])
        
        st.json({
            "Session ID": st.session_state.get('session_id', 'No generado'),
            ...
            "Page Views": st.session_state.get('page_views', 0),
            "Session Start": st.session_state.get('session_start_time', 'No iniciada').__str__()
        })
...
def create_traffic_sources_chart(df_traffic):
    ...
    fig = px.bar(
        ...
        y='Fuente/Medio',
        orientation='h',
        # title="🚀 Top 10 Fuentes de Tráfico por Sesiones",
...
def create_traffic_medium_pie_chart(df_summary):
    """Crear gráfico circular de distribución por medio"""
    if df_summary.empty:
        return None
    
    fig = px.pie(
        df_summary,
        values='Sesiones Totales',
        names='Medio',
        title="📊 Distribución de Sesiones por Medio de Tráfico",
        color_discrete_sequence=px.colors.qualitative.Set3
    )
    
    fig.update_traces(
        textinfo='percent+label+value',
        hovertemplate='%{label}<br>Sesiones: %{value:,.0f}<br>%{percent}<extra></extra>'
    )
...
def create_traffic_sources_scatter(df_sources):
    """Crear gráfico de dispersión de fuentes vs usuarios y sesiones"""
    if df_sources.empty:
        return None
    ...
    fig = px.scatter(
        ...
        y='Usuarios Activos',
        size='Sesiones',
        color='Usuarios por Sesión',
        hover_name='Fuente',
        title="🎯 Rendimiento de Fuentes: Usuarios vs Sesiones",
...
def create_traffic_metric_card_html(title, value, icon="🚀"):
    """Crear HTML para tarjetas de métricas de tráfico"""
    return f"""
...
def main_dashboard():
    ...
    with st.sidebar:
        ...
        if available_dates:
            ...
            # Calcular default (últimos 7 días)
            default_start = available_dates[-7] if len(available_dates) >= 7 else available_dates[0]
            ...
            with col1:
                fecha_inicio = st.selectbox(
                    ...
                    index=available_dates.index(default_start)
                )

            with col2:
                # Mostrar solo fechas posteriores a la de inicio
                fin_options = [d for d in available_dates if d >= fecha_inicio]
                fecha_fin = st.selectbox(
                    "Fecha Fin:",
                    options=fin_options,
                    index=len(fin_options) - 1
                )
    ...
    with st.spinner("📊 Cargando datos del período seleccionado..."):
        ...
        # print(df_hourly)
        # AGREGAR ESTAS LÍNEAS DESPUÉS DE df_hourly = get_hourly_data_range(db, fecha_inicio, fecha_fin)
        df_traffic_sources = get_traffic_sources_range(db, fecha_inicio, fecha_fin)
        # print(df_traffic_sources)
        df_traffic_summary = get_traffic_sources_summary(db, fecha_inicio, fecha_fin)
        df_top_sources = get_top_traffic_sources(db, fecha_inicio, fecha_fin)
        # print(df_traffic_sources) 
        # print(df_top_sources)
    ...
    with st.expander("📊 Ver Datos Detallados"):
        ...
        # NUEVA TAB PARA FUENTES DE TRÁFICO
        with tab7:
            if not df_traffic_sources.empty:
                ...
                st.info("No hay datos de fuentes de tráfico de naaaaaaaaaaaada")
    ...
    # =============================================================================
# 🚀 4. ANÁLISIS DE FUENTES DE TRÁFICO (NUEVA SECCIÓN)
# =============================================================================
    ...
    if not df_traffic_sources.empty:
        ...
        # with col1:
        #     total_traffic_sessions = df_traffic_sources['Sesiones'].sum()
        #     st.markdown(create_traffic_metric_card_html(
        #         "Total Sesiones", 
        #         format_number_complete(total_traffic_sessions),
        #         "📈"
        #     ), unsafe_allow_html=True)
        
        # with col2:
        #     unique_sources = len(df_traffic_sources['Fuente/Medio'].unique())
        #     st.markdown(create_traffic_metric_card_html(
        #         "Fuentes Únicas", 
        #         format_number_complete(unique_sources),
        #         "🌐"
        #     ), unsafe_allow_html=True)
        
        # with col3:
        #     total_traffic_users = df_traffic_sources['Usuarios Activos'].sum()
        #     st.markdown(create_traffic_metric_card_html(
        #         "Usuarios por Tráfico", 
        #         format_number_complete(total_traffic_users),
        #         ""
        #     ), unsafe_allow_html=True)
        
        # with col4:
            # if not df_traffic_summary.empty:
            #     main_medium = df_traffic_summary.iloc[0]['Medio']
            #     st.markdown(create_traffic_metric_card_html(
            #         "Medio Principal", 
            #         main_medium,
            #         "🎯"
        ...
        # with col2:
        #     if not df_traffic_summary.empty:
        #         medium_pie = create_traffic_medium_pie_chart(df_traffic_summary)
        #         if medium_pie:
        #             st.plotly_chart(medium_pie, use_container_width=True)
        
        # # Análisis detallado de fuentes específicas
        # if not df_top_sources.empty:
        #     st.subheader("🎯 Análisis Detallado de Fuentes")
        #     sources_scatter = create_traffic_sources_scatter(df_top_sources)
        #     if sources_scatter:
        #         st.plotly_chart(sources_scatter, use_container_width=True)
