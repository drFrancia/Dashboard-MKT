...
# =============================================================================
# FUNCIONES PARA BÚSQUEDA INTERNA
# =============================================================================

@st.cache_data(ttl=300)
def get_internal_search_range(_db, fecha_inicio, fecha_fin, limit=20):
    """Obtener búsquedas internas en un rango de fechas"""
    sql = f"""
    SELECT 
        searchTerm as "Término de Búsqueda",
        pagePath as "Página de Búsqueda",
        SUM(sessions) as "Sesiones",
        SUM(activeUsers) as "Usuarios Activos",
        SUM(screenPageViews) as "Vistas de Página",
        AVG(bounceRate) as "Tasa de Rebote"
...
@st.cache_data(ttl=300)
def get_internal_search_summary(_db, fecha_inicio, fecha_fin):
    """Obtener resumen de búsquedas internas"""
    sql = f"""
    SELECT 
        searchTerm as "Término",
        COUNT(DISTINCT pagePath) as "Páginas Diferentes",
        SUM(sessions) as "Sesiones Totales",
        SUM(screenPageViews) as "Vistas Totales"
    FROM {TABLE_CONFIG['busqueda_interna']} 
    WHERE fecha BETWEEN %s AND %s
    AND searchTerm IS NOT NULL 
    AND searchTerm != ''
...
@st.cache_data(ttl=300)
def get_utm_tracking_range(_db, fecha_inicio, fecha_fin, limit=20):
    """Obtener datos de seguimiento UTM (campañas)"""
    sql = f"""
    SELECT 
...
def compare_search_terms(df1, df2, top_n=10):
    """Comparación de términos de búsqueda - CORREGIDO para incluir todos los términos"""
    if df1.empty or df2.empty:
        return None
    
    # Obtener TODOS los términos únicos de ambos períodos
    all_terms = set(list(df1['Término de Búsqueda'].unique()) + list(df2['Término de Búsqueda'].unique()))
    
    comparison = {}
    
    # Comparar todos los términos
...
def compare_geography(df1, df2, top_n=10):
    """Comparación de datos geográficos - CORREGIDO para incluir todos los países"""
    if df1.empty or df2.empty:
        return None
    
    # Agrupar TODOS los países de ambos períodos
...
def main_dashboard():
    ...
    if comparison_mode:
        ...
        with st.spinner("📊 Cargando datos del período seleccionado..."):
            ...
            df_internal_search_summary = get_internal_search_summary(db, fecha_inicio, fecha_fin)
            df_utm = get_utm_tracking_range(db, fecha_inicio, fecha_fin)
    ...
    # =============================================================================
    # TARJETAS DE RESUMEN DE TÉRMINOS DE BÚSQUEDA
    # =============================================================================
    
    if not df_search_terms.empty:
        st.markdown("---")
        st.subheader("🔍 Resumen de Términos de Búsqueda")
        
        col1, col2, col3, col4 = st.columns(4)
        
        total_search_sessions = df_search_terms['Sesiones'].sum()
        total_search_users = df_search_terms['Usuarios Activos'].sum()
        total_search_views = df_search_terms['Vistas de Página'].sum()
        avg_search_duration = df_search_terms['Duración Promedio'].mean()
        
        with col1:
            st.markdown(create_metric_card_html(
                "Sesiones desde Búsquedas", 
        ...
        with col2:
            st.markdown(create_metric_card_html(
                "Usuarios desde Búsquedas", 
                format_number_complete(total_search_users),
                "👥"
            ), unsafe_allow_html=True)
        ...
        with col3:
            st.markdown(create_metric_card_html(
                "Páginas Vistas", 
                format_number_complete(total_search_views),
                "📄"
            ), unsafe_allow_html=True)
        
        with col4:
            # Convertir duración promedio a minutos y segundos
            duracion_mins = int(avg_search_duration // 60)
            duracion_segs = int(avg_search_duration % 60)
            st.markdown(create_metric_card_html(
                "Duración Promedio", 
                f"{duracion_mins}m {duracion_segs}s",
                "⏱️"
            ), unsafe_allow_html=True)
        
        # Mostrar top 5 términos de búsqueda
        with st.expander("🏆 Top 5 Términos de Búsqueda", expanded=True):
            df_top5 = df_search_terms.head(5)
            for idx, row in df_top5.iterrows():
                col_a, col_b, col_c = st.columns([3, 1, 1])
                with col_a:
                    st.markdown(f"**{idx+1}. {row['Término de Búsqueda']}**")
                with col_b:
                    st.metric("Sesiones", f"{int(row['Sesiones']):,}")
                with col_c:
                    st.metric("Usuarios", f"{int(row['Usuarios Activos']):,}")
    ...
    with st.expander("📊 Ver Datos Detallados"):
        ...
        with tab4:
            if not df_pages.empty:
                ...
                st.dataframe(df_pages_display, use_container_width=True)
        ...
        with tab7:
            if not df_internal_search.empty:
                st.subheader("🔍 Datos de Búsqueda Interna")
                # Formatear Tasa de Rebote si existe
                df_internal_display = df_internal_search.copy()
                if 'Tasa de Rebote' in df_internal_display.columns:
                    df_internal_display['Tasa de Rebote'] = (df_internal_display['Tasa de Rebote'] * 100).round(2).astype(str) + '%'
                st.dataframe(df_internal_display, use_container_width=True)
                
                # Opción de descarga
                csv = df_internal_search.to_csv(index=False)
                st.download_button(
                    label="💾 Descargar datos de búsqueda interna (CSV)",
                    data=csv,
                    file_name=f'busqueda_interna_{fecha_inicio}_{fecha_fin}.csv',
                    mime='text/csv'
                )
            else:
                st.info("No hay datos de búsqueda interna")
        ...
        with tab9:
            if not df_utm.empty:
                ...
                st.dataframe(df_utm_display, use_container_width=True)
    ...
    # =============================================================================
    # BÚSQUEDA INTERNA (NUEVA SECCIÓN)
    # =============================================================================
    
    st.header("🔍 Análisis de Búsqueda Interna del Sitio")
    
    if not df_internal_search.empty:
        col1, col2 = st.columns([2, 1])
        
        with col1:
            st.subheader("📊 Términos de Búsqueda Interna Más Utilizados")
            
            # Mostrar tabla de búsquedas internas
            df_internal_display = df_internal_search.head(15).copy()
            
            # Formatear valores y limpiar NaN
            if 'Tasa de Rebote' in df_internal_display.columns:
                # Reemplazar NaN con 0 antes de formatear
                df_internal_display['Tasa de Rebote'] = df_internal_display['Tasa de Rebote'].fillna(0)
                df_internal_display['Tasa de Rebote'] = (df_internal_display['Tasa de Rebote'] * 100).round(2).astype(str) + '%'
            
            st.dataframe(df_internal_display, use_container_width=True)
            
            # Crear gráfico de barras para términos de búsqueda interna
            if not df_internal_search_summary.empty:
                fig_internal = px.bar(
                    df_internal_search_summary.head(10),
                    x='Sesiones Totales',
                    y='Término',
                    orientation='h',
                    title='Top 10 Términos de Búsqueda Interna por Sesiones',
                    color='Sesiones Totales',
                    color_continuous_scale='Viridis'
                )
        ...
        with col2:
            if not df_internal_search_summary.empty:
                st.subheader("📈 Resumen de Búsqueda Interna")
                
                # Calcular estadísticas
                total_internal_searches = df_internal_search_summary['Sesiones Totales'].sum()
                total_unique_terms = len(df_internal_search_summary)
                avg_pages_per_term = df_internal_search_summary['Páginas Diferentes'].mean()
                top_term = df_internal_search_summary.iloc[0]['Término'] if len(df_internal_search_summary) > 0 else 'N/A'
                top_term_sessions = df_internal_search_summary.iloc[0]['Sesiones Totales'] if len(df_internal_search_summary) > 0 else 0
                
                st.markdown(f"""
                <div class="search-terms-card">
                    <h4>📊 Métricas de Búsqueda Interna</h4>
                    <p><strong>Total de búsquedas:</strong> {total_internal_searches:,}</p>
                    <p><strong>Términos únicos:</strong> {total_unique_terms}</p>
                    <p><strong>Páginas promedio por término:</strong> {avg_pages_per_term:.1f}</p>
                    <hr>
                    <p><strong>Término más buscado:</strong></p>
                    <p style="font-size: 1.1em;">"{top_term}"</p>
                    <p><strong>{top_term_sessions:,} sesiones</strong></p>
                </div>
                """, unsafe_allow_html=True)
                
                # Top páginas donde se realizan búsquedas
                st.subheader("📄 Páginas de Búsqueda")
                top_search_pages = df_internal_search.groupby('Página de Búsqueda')['Sesiones'].sum().sort_values(ascending=False).head(5)
                for page, sessions in top_search_pages.items():
                    page_display = page[:50] + "..." if len(page) > 50 else page
                    st.markdown(f"• **{page_display}**: {sessions:,} búsquedas")
            else:
                st.info("No hay datos de resumen disponibles")
    else:
        st.info("⚠️ No hay datos de búsqueda interna disponibles para el período seleccionado")
    ...
    if not df_utm.empty and not df_utm_summary.empty:
